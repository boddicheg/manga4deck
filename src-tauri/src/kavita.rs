...
pub static KAVITA: OnceCell<Mutex<Kavita>> = OnceCell::const_new();
...
impl Kavita {
    ...
    pub async fn reconnect_with_creds(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let response = client.post(format!("http://{}/api/Account/login", ip))
            .json(&json!({
                "username": username,
                "password": password,
                "apiKey": api_key
    ...
    pub async fn initialize() -> Result<(), Box<dyn std::error::Error>> {
        let kavita = Self::new().await?;
        KAVITA.get_or_init(|| async { Mutex::new(kavita) }).await;
        Ok(())
    }
}
